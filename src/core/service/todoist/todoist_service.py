...
class TodoistService(BaseService):
    ...
    def create_new_project(self, body: CreateProjectRequest) -> ResponseContainer[ProjectData]:
    ...
    def get_all_projects(self) -> ResponseContainer[List[ProjectData]]:
        with self._context_step("get all projects"):
            ...
            return ResponseContainer(
                status_code=response.status_code,
                headers=dict(**response.headers),
                content=[ProjectData(**item) for item in response.json()] if response.ok else response.text
            )
